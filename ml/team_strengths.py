...
class TeamStrengthCalculator:
    def __init__(self, df_games):
        ...
        self.teams = sorted(list(df_games["Home"].unique()))
        self.ind_of_team = {team : ind for ind, team in enumerate(self.teams)}
    ...
    def process_day(self):
        if self.next_date_ind < len(self.game_dates):
            for _, row in self.df_games[self.df_games["Date"] == self.game_dates[self.next_date_ind]].iterrows():
                # 0 to 29 for teams, 30 for home court (always 1), 31 for rest differential
                has_rest_data = not (pd.isna(row["RestHome"]) or pd.isna(row["RestOpp"]))
                if has_rest_data:
                    row_X = np.zeros(32)
                    row_X[self.ind_of_team[row["Home"]]] = 1
                    row_X[self.ind_of_team[row["Opp"]]] = -1
                    row_X[30] = 1
                    row_X[31] = row["RestHome"] - row["RestOpp"]
